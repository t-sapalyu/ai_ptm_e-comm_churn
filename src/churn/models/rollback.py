...
The registry already stores every historical version of ``churn_classifier``.
This script is a thin CLI over the MLflow client so a rollback is one command.
...
Promote a specific version back to Production::
    ...
    python -m churn.models.rollback --version 7

Whichever version was previously in Production is moved to ``Archived`` so
exactly one version is in Production at any time.
"""
...
def list_versions() -> None:
    """Print every registered version of the model with its stage and metrics."""
    client = MlflowClient()
    versions = client.search_model_versions(f"name='{config.REGISTERED_MODEL_NAME}'")

    if not versions:
        print(f"No versions registered yet for '{config.REGISTERED_MODEL_NAME}'.")
        return
    ...
    for v in versions:
        ...
        print(f"{v.version:<8}{v.current_stage:<14}{v.run_id:<34}{val_f1:<10.3f}")


def promote(version: str) -> None:
    """Transition the given version to Production, archiving the current one."""
    client = MlflowClient()
    ...
    if target.current_stage == "Production":
        logger.info("Version %s is already in Production. Nothing to do.", version)
        return
