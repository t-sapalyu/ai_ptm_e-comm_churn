...
def load_production_model() -> object:
    ...
    versions = client.get_latest_versions(
        config.REGISTERED_MODEL_NAME, stages=["Production"]
    )
    if not versions:
        raise RuntimeError(
...
def predict(input_path: Path, output_path: Path) -> None:
    """Score rows in ``input_path`` and write result to ``output_path``.
    ...
    if missing_features:
        raise ValueError(
            ...
            f"{missing_features}. Expected at least: {config.ALL_FEATURES}"
        )
    if config.ID_COL not in df.columns:
        raise ValueError(
            f"Input CSV is missing the customer identifier "
            f"'{config.ID_COL}'. Predictions without an ID cannot "
            f"be joined back to the CRM."
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output", type=Path, required=True, help="Output CSV."
    )
