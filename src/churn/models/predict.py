...
Loads ``models:/churn_classifier/Production`` from the MLflow registry, scores
each row, and writes a CSV that contains:

* ``CustomerID``         — carried through unchanged so the marketing team can
  join the predictions back to the CRM.
* All input columns      — kept so the output is self-contained.
* ``churn_prediction``   — the predicted class (0 / 1).
* ``churn_probability``  — the predicted probability of churn.

The model itself only ever sees the columns in :data:`churn.config.ALL_FEATURES`
— ``CustomerID`` is **not** one of them.
"""
