"""Train three churn models and log each run to MLflow.

Run with::

    python -m churn.models.train                  # train all three
    python -m churn.models.train --model xgboost  # train only one

Each call opens a parent MLflow run and one nested child run per model. The
fitted pipeline (preprocessor + classifier) is logged and registered under the
name ``churn_classifier``. The child run with the best validation F1 has its
registered version transitioned to ``Staging``.

For full reproducibility, every parent run is tagged with the dataset SHA-256
fingerprint, the current git commit, and the versions of scikit-learn and
xgboost. This means a model can always be traced back to the exact data and
...
def get_model(name: str) -> tuple[Any, dict[str, Any]]:
    """Return an unfit estimator and the hyperparameters used to construct it.

    The hyperparameters are returned separately so they can be logged to MLflow
    without having to introspect the estimator afterwards.

    Parameters
    ----------
    name : str
        One of ``"logreg"``, ``"random_forest"``, ``"xgboost"``.
    ...
    ------
    ValueError
        If ``name`` is not a known model identifier.
    """
    if name == "logreg":
        params = {
            "C": 1.0,
...
def load_splits() -> tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series]:
    ...
    Only :data:`config.ALL_FEATURES` columns are returned as ``X``; the
    customer ID stays out of the model's input. The target column comes back
    as ``y``.
    """
